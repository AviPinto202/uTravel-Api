...
--- v
Create proc ForgotPassword_Business
	@Email varchar(50),
	@New_Password NVarchar(50)
as
    if exists (Select * from [dbo].[Business] where [Email] = @Email)
	    Update Business
        Set [Password] = @New_Password
        where [Email] = @Email
go
...
as
	if exists (Select * from [dbo].[Cities] where [City_Name] = @City_Name)
		...
		begin
			insert into [dbo].[Cities] ([City_Name], [Country], [Information], [language], [Currency], [Weather], [Title], [Rating_Star], [Image])
				values (@City_Name , @Country , @Information , @language , @Currency , @Weather, @Title, @Rating_Star, @Image)
...
as
	Update Cities
	Set [City_Name] = @City_Name , [Country] = @Country , [Information] = @Information , [language] = @language,
	[Currency] = @Currency , [Weather] = @Weather , [Title] = @Title , [Rating_Star] = @Rating_Star, [Image] = @Image
...
as
	if NOT EXISTS (Select * from [dbo].[ItemInOrder] where [Attraction_Id] = @Attraction_Id)
		begin
...
--Create proc SelectPriceByBusinessId
--	@Business_Id int
--as
--	Select [Attraction_Id] and [Quantity] from [dbo].[Orders] from [dbo].[ItemInOrder] where [Business_Id] = @Business_Id
--	Sum() 
--go